...
pkt=IP(dst=dst_ip)/TCP(sport=src_port,dport=dst_port,flags="F")  # 构建数据包，设置FIN标志位
tcp_fin_scan_resp = sr1(pkt,timeout=10)  # 发送1个数据包，此变量是接收包
if(str(type(tcp_fin_scan_resp))=="<type 'NoneType'>"):
    print("Open or Filtered or non-existent")  # 无响应，说明端口处于开启/过滤状态或者目标主机关闭/不存在
elif(tcp_fin_scan_resp.haslayer(TCP)):
    if (tcp_fin_scan_resp.getlayer(TCP).flags == 0x14):
        print("Closed")
