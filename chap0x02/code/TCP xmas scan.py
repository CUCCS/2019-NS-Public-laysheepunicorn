...
pkt=IP(dst=dst_ip)/TCP(sport=src_port,dport=dst_port,flags="FPU")  # 构建数据包，设置FPU三个标志位
tcp_xmas_scan_resp = sr1(pkt,timeout=10)  # 发送1个数据包，此变量是接收包
if(str(type(tcp_xmas_scan_resp))=="<type 'NoneType'>"):
    print("Open or Filtered or non-existent")  # 无响应，说明端口处于开启/过滤状态或者目标主机关闭/不存在
elif(tcp_xmas_scan_resp.haslayer(TCP)):
    if (tcp_xmas_scan_resp.getlayer(TCP).flags == 0x14):
        print("Closed")
